...
impl<T: io::Read + io::Write> Xmodem<T> {
    ...
    fn expect_byte(&mut self, byte: u8, expected: &'static str) -> io::Result<u8> {
        match self.read_byte(false)? {
            CAN if byte != CAN => Err(io::Error::new(io::ErrorKind::ConnectionAborted, "received CAN")),
            b if b == byte => Ok(byte),
    ...
    pub fn read_packet(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        if buf.len() < 128 {                
            return Err(io::Error::new(io::ErrorKind::UnexpectedEof, "Too small buffer"));
        }
        ...
        match self.read_byte(true)? {
            ...
            SOH => {
                ...
                self.expect_byte_or_cancel(255 - packet, "Wrong packet ones' complement")?;

                self.inner.read_exact(&mut buf[..128])?;
                
                (self.progress)(Progress::Packet(self.packet));

                let checksum: u8 = buf.iter().fold(0, |a: u8, &e| a.wrapping_add(e));
            ...
            _ => {
                return Err(io::Error::new(io::ErrorKind::InvalidData, "Invalid data received"));
            }
        }
    }
    ...
    pub fn write_packet(&mut self, buf: &[u8]) -> io::Result<usize> {
        ...
        if buf.len() == 0 {
            self.write_byte(EOT)?;
            self.expect_byte(NAK, "NAK expected from receiver")?;
            self.write_byte(EOT)?;
            self.expect_byte(ACK, "ACK expected from receiver")?;
            self.started = false;
            return Ok(0);
        }
        ...
        match self.read_byte(true)? {
            ...
            NAK => return Err(io::Error::new(io::ErrorKind::InvalidData, "Received NAK")),
